...
public class Simulateur implements Simulable {
    ...
    /**
     * Ajourte un événement à la liste des événements
     * 
    ...
    /**
     * Retourne vrai si il ne reste plus d'événements à traiter
     * 
     * @return vrai si il ne reste plus d'événements à traiter, faux sinon
     */
    ...
    /**
     * Dessine les cases qui prennent en compte les bords en fonction de la nature
     * de la case et de ses voisins.
     * 
     * @param c
     * @param sprites
     * @param carte
    ...
    private String[] drawElementBord(Case c, String[] sprites, Carte carte, boolean coin) {
        ...
        if (coin) {
            ...
            if (s1 == sprites[0] && lig > 0 && col > 0
                    && carte.getCase(lig - 1, col - 1).getNature() != c.getNature()) {
                s1 = sprites[12];
            }

            if (s2 == sprites[0] && lig > 0 && col < carte.getNbColonnes() - 1
                    && carte.getCase(lig - 1, col + 1).getNature() != c.getNature()) {
                s2 = sprites[11];
            }
            if (s3 == sprites[0] && lig < carte.getNbLignes() - 1 && col > 0
                    && carte.getCase(lig + 1, col - 1).getNature() != c.getNature()) {
                s3 = sprites[10];
            }
            if (s4 == sprites[0] && lig < carte.getNbLignes() - 1 && col < carte.getNbColonnes() - 1
                    && carte.getCase(lig + 1, col + 1).getNature() != c.getNature()) {
                s4 = sprites[9];
            }
        }
    ...
    private void drawCase(int xCase, int yCase, Random random, Case c, Carte carte) {
        ...
        if (c.getNature() == NatureTerrain.TERRAIN_LIBRE) {
            ...
            gui.addGraphicalElement(
                    new ImageElement(adjustedX + halfSize, adjustedY + halfSize, s4, halfSize, halfSize, null));
    ...
    private void drawIncendies() {
        ...
        for (Incendie incendie : this.model.getIncendiesParCase().values()) {
            if (incendie.getQuantiteEau() > 0) {
                ...
                gui.addGraphicalElement(
                        new ImageElement(adjustedX, adjustedY, sprite, taillePixelIncendie, taillePixelIncendie, null));
    ...
    private void drawRobot(Robot robot) {
        ...
        int adjustedY = y - halfSize;
        gui.addGraphicalElement(
                new ImageElement(adjustedX, adjustedY, sprite, taillePixelCases, taillePixelCases, null));
    }
    ...
    /**
     * Redémarre la simulation en réinitialisant les robots, les incendies et les
     * événements.
     */
    @Override
    public void restart() {
